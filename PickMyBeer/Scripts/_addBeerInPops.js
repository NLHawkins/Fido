$().ready(function () {
    ...
    var showBeersInPops = function (beerInPops) {
        var ctrl = $(".beersInPops")
        ctrl.empty();
        for (idx in beerInPops) {
            var name = beerInPops[idx].Beer.Name;
            var beerId = beerInPops[idx].Beer.Id
    ...
    var getBeersInPops = function () {
        $.get('http://localhost:54414/BarUser/GetBeersInPops', function (resp) {
            console.log(resp)
            showBeersInPops(resp)
        });
    }
    var showSBeers = function (beers) {
        ...
        for (idx in beers) {
            ...
            ctrl.append("<tr><td>" + name + "</td><td>" + desc + "</td><td>" + style + "</td><td>" + brewery + "</td><td><button data-bIdx =" + idx + " class='choosePopBeer'>Choose</button></td></tr>");
        };
        ...
        $(".choosePopBeer").click(function () {
            ...
            $.ajax({
                url: 'http://localhost:54414/Beer/AddBeerInPops',
